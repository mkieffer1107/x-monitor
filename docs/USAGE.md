...
Detailed setup and usage notes for `/Users/mkieffer/programming/x-monitor`.
...
4. If using AI analysis with default AI providers, set at least one provider key:
...
Enable persistent session logs (feed lines, errors, URLs):
...
- Monitor definitions persist in `x-monitor-state.json` by default.
- Stream reconnects automatically on disconnect.
- Deactivated targets stay saved but do not stream until re-activated.
- If an AI provider has no configured key, set an API key override in Add Monitor.
